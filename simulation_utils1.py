...
def extract_ans(res: str) -> str:
    ...
    # 2) If no “answer is” AND no A:/Answer: prefix, abstain
    if "A:" not in res and "Answer:" not in res:
        logging.debug(f"wrong format, no A:/Answer: {res!r}")
        return "Abstain"

    # 3) Refusal tokens
    if "<answer>" in res or "I'm sorry" in res:
        logging.debug(f"Abstain from refusal: {res!r}")
        return "Abstain"

    # 4) Fallback strip prefixes + punctuation
    ans = (
        res.split("\n", 1)[0]
           .split("Confidence", 1)[0]
           .split("confidence", 1)[0]
           .replace("A:", "")
           .replace("Answer:", "")
           .strip()
    ...
    if not ans:
        logging.debug(f"No fallback answer extracted: {res!r}")
        return "Abstain"

    logging.debug(f"extracted via fallback: {ans!r}")
    return ans
