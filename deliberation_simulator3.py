...
def construct_stances(
    ...
    filter_abstain: Optional[bool] = False,
...
def agents_deliberation_single_thread(
    ...
    for _, record in tqdm(df.iterrows()):
        ...
        print(f"[DEBUG] QID {record['qid']} → raw votes ({len(original_votes)}):\n{original_votes}\n")
        print(f"[DEBUG] About to call construct_stances with {len(original_votes)} votes")
        print(f"QID {record['qid']} → stances={len(stances)}")
        if not len(stances):
        ...
        summary_path = output_filepath / "summary_vqarad_f.csv"
...
def main(args):
    ...
    if args.input_dataset == "pathrad":
        ...
        dev_data  = test_data.iloc[: args.dev_sample_size] \
                            .reset_index(drop=True)
    # Stage 1 agents
    selection = {"cot": 1, "knowledge": 1}
    expert_agent_pool = populate_expert_agents(
        selection,
        model_ensemble=True,      # turn on multi-model ensemble
