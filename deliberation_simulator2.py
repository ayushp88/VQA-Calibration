...
def populate_general_agents(
    ...
    """
    Create `size` many generic vision–language agents, all using the same HF
    checkpoint via BaseAgent (no special CoT/POT/Search prompting).
    """
    # 1) Load the HF model once
    ...
    # 2) Wrap it in BaseAgent for each slot
...
def revote(
    ...
    for agent, initial_conf, original_observations, new_observations, image in mappings:
        ...
        conf_prompt = (
            f"Recall your original confidence for your answer was {initial_conf:.2f}. "
            f"Given the rationale:\n'''{rationale}'''\n"
        ...
        try:
            conf_parsed = float(
                revote_conf.split("Confidence:")[-1].split()[0].strip()
            )
        except ValueError:
            logging.debug(f"conf_parsed failed on: {revote_conf}")
...
def agents_deliberation_single_thread(
    ...
    for _, record in tqdm(df.iterrows()):
        ...
        correct_choice = record["reference_answers"]
        if not final_ans_set:
            continue  
        final_majority = sorted(list(final_ans_set), key=lambda t: t[3])[-1]
        summary_path = output_filepath / "summary_social_qwenfinal.csv"
        ...
        with open(summary_path, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([record["qid"], final_majority[0], final_majority[1], correct_idx,correct_choice])
        print(final_majority[0], final_majority[1])
...
def main(args):
    ...
    csv_path = os.path.join(args.memory_filepath, "summaryqwn.csv")
    ...
    if args.input_dataset == "scienceqa":
        ...
        ds = ds.filter(
            lambda ex: ex.get("subject", "").lower() == "social science"
            and ex.get("image") is not None
        )
    ...
    # Stage 2 agents (same size as Stage 1 agents), fixed for all queries
    #general_agent_pool = populate_general_agents(args.group_size)    
    #qwen_general  = populate_general_agents(2, model_name="Qwen/Qwen2.5-VL-3B-Instruct", use_vllm=args.vllm)
    ##gemma_general = populate_general_agents(2, model_name="google/gemma-3-4b-it",       use_vllm=args.vllm)
    #general_agent_pool = qwen_general + gemma_general
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-input_dataset",
        type=str,
        choices=["triviaqa-dev", "sciq-valid", "truthfulqa", "math-test-prm800k", "gsm8k-test", "WikiLingua-1000-chn-eng", "theoremqa-test", "ambigqa", "gpqa_diamond", "dateUnd", "prfLaw","scienceqa", "Biz-Ethics"],
