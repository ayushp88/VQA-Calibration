...
class BaseAgent:
    def __init__(
        ...
        self.memory = BaseAgentMemory(
            llm=ChatOpenAI(temperature=0.9, max_tokens=1024, max_retries=50, request_timeout=800, openai_api_key="key"  ),
        ...
        self.default_trigger_prompt = (
            "Answer the given question in yes or no only "
    ...
    def self_deliberate_with_pretrained_instance(
        self,
        query: str,
        image: Optional[Image.Image] = None,
        use_hf_chat_template: bool = False
    ) -> Tuple[float, str]:
        """Zero‑shot + one‑shot example prompting with format enforcement."""
        processor, model = self.pretrained

        # 1) System message enforcing format and providing one‑shot example
        from langchain.schema import SystemMessage
        example = (
            "Example:\n\n"
            "Answer: no\n"
            ...
            "Ability: 0.90\n"
            "Confidence: 0.87\n\n"
            "Now given this new question… Answer in the same format:\n"
        )
        sys_msg = SystemMessage(content=example +
                                f"Question: “{query}”")

        # 2) Build the rest of the chat prompt
        if use_hf_chat_template:
            ...
            # wrap the system message plus question via your model‐specific template
            chat_prompt = get_model_specific_chat_template(
                self.model_type,
                sys_msg.content
            )

        # 3) Call the VLM to generate answer + joint probability
        prob, res = causal_lm_generate(
            processor,
            model,
            chat_prompt,
            image,
            return_joint_prob=True
        )

        # 4) (Optional) Validate format and re‑prompt once if needed
...
"""
    ...
    # If this agent was built from HF (so has a (processor, model) tuple in self.pretrained)
        if hasattr(self, "pretrained"):
            processor, model = self.pretrained
            # Build the raw user‐prompt exactly as before
            user_msg = (
                f"You are participating in a debate on the question: '{q}'\n"
                f"Your assigned stance on the question is '{assigned_stance}'\n"
            ...
            # Wrap via your model‐specific chat template
            chat_prompt = get_model_specific_chat_template(self.model_type, user_msg)
            # Call your VLM generator (no image here, so pass image=None)
            _, res = causal_lm_generate(
                processor,       # the AutoProcessor
                model,           # the Gemma3ForConditionalGeneration (or similar)
                chat_prompt,
                image=image ,
                return_joint_prob=False
            )
            # strip off any “Argument:” label
            argument = res.replace("Argument:", "").strip()
            logging.debug(f"generate_argument ({self.model_type})\n{user_msg}\n→ {argument}")
            return argument
