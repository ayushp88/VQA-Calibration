...
TEST_HF_MODELS = {
    ...
    "google":["gemma-3-4b-it", "medgemma-4b-it"],
    "qwen":["Qwen2.5-VL-3B-Instruct"],
    "microsoft":["Phi-4-multimodal-instruct"],
    "OpenGVLab":["InternVL2_5-4B"],
    "LiAutoAD":["Ristretto-3B"],
...
def extract_ans(res: str) -> str:
    # 1) If there's a {...} JSON blob anywhere, parse it and return its "Answer"
    ...
    # 2) Strip out markdown fences and any filler lines (like "hello")
    cleaned = re.sub(r"```(?:json)?", "", res)
    lines = [
        line.strip()
        for line in cleaned.splitlines()
        if line.strip() and not line.strip().lower().startswith("hello")
    ]
    if not lines:
        return "Abstain"

    # 3) Take the first non‑empty line and remove any A:/Answer: prefix
    first = lines[0]
    for prefix in ("A:", "Answer:"):
        if first.lower().startswith(prefix.lower()):
            first = first[len(prefix):].strip()
            break

    return first or "Abstain"
...
def get_model_specific_chat_template(model_type: str, user_msg: str) -> str:
    if model_type == "meta-llama/Llama-2":
        return (
    ...
    elif "gemma" or "qwen" or "phi" in model_type.lower():
        # no special tokens—just pass through the prompt (including <image> if you're using it)
        return user_msg  
    else:
...
def generate_feedback(
        ...
        nli_tokenizer: Any,
        nli_classifier: Any,
        rating_scales: Optional[List[str]] = ["bad", "modest", "good", "excellent", "excelent"],
