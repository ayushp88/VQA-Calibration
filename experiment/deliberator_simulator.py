...
def agents_deliberation_single_thread(
    ...
    for _, record in tqdm(df.iterrows()):
        ...
        summary_path = output_filepath / "summary_social.csv"
...
def main(args):
    ...
    csv_path = os.path.join(args.memory_filepath, "summary.csv")
...
# Keep only examples whose `subject` matches:
        ds_yesno = load_dataset("flaviagiammarino/vqa-rad", split="test").filter(lambda ex: ex.get("answer", "").strip().lower() in ("yes", "no"))
        ...
        # ── 3) Build DataFrame ─────────────────────────────────────────────────────
        records = []
        for idx, ex in enumerate(ds_yesno):
            q = ex["question"].strip()
            if not q:
                continue
            records.append({
    ...
    general_agent_pool = populate_general_agents(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-input_dataset",
        type=str,
        choices=["triviaqa-dev", "sciq-valid", "truthfulqa", "pathrad","math-test-prm800k", "gsm8k-test", "WikiLingua-1000-chn-eng", "theoremqa-test", "ambigqa", "gpqa_diamond", "dateUnd", "prfLaw","scienceqa", "Biz-Ethics"],
